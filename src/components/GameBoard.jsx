...
import { GameModal, GridContainer } from "./";
import {
  BOARD_SIZE,
  WIN_NUMBER,
  MAX_NUMBER,
...
import { getDirection, getPosition } from "../utils/touchEvent.js";
import {
  getRow,
  getColumn,
  getNewTile,
...
export default function GameBoard({
    ...
    (direction) => {
      ...
      if (isChanged(newArray)) {
        ...
        if (newTile) {
          ...
          setEarnedPoints((points) => points + totalAddedScore);
          setNumbers([...newArray]);
  ...
  function getLine({ row, col }, isUpDown) {
    return numbers.filter(
      (tile, index) =>
        tile.number &&
        (isUpDown ? getColumn(index) === col : getRow(index) === row)
    );
  }
  ...
  return (
    <Container onTouchStart={handleTouchStart} onTouchEnd={handleToucheEnd}>
